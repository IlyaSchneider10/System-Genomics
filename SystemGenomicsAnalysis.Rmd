...
meta_genes$highvar <- meta_genes$ensembl_gene_id_version %in% rownames(var_genes)[which(var_genes$padj<1)] #this line was cut in the tutorial so i completed it, but no idea if its right
...
expr_combat<-ComBat_seq(counts=expr,batch=meta$Individual)

corr_spearman_highvar <- cor(expr_combat[meta_genes$highvar,], method = "spearman")
hcl_spearman_highvar <- hclust(as.dist(1 - corr_spearman_highvar))
layout(matrix(1:2,nrow=1))
...
pca_highvar <- prcomp(log1p(t(expr_combat[meta_genes$highvar,])), center = TRUE, scale. = TRUE)
ggplot(data.frame(pca_highvar$x, meta)) +
geom_point(aes(x = PC1, y = PC2, color = Sex, shape = disease_state), size = 5)
