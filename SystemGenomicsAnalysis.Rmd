...
```{r}
#ANCOVA test between the different sexes with bonferoni correction for multiple testing
pvals <- pbapply(expr[meta_genes$expressed,], 1, function(e){
  dat <- data.frame(y = log1p(e),
                    meta)
  m1 <- lm(y ~ disease_state + Individual, data = dat)
...
fc <- pbapply(expr[meta_genes$expressed,], 1, function(e){
  avg_disease <- tapply(log1p(e), meta$disease_state, mean)
  return(exp(max(avg_disease) - min(avg_disease)))
})
...
ggplot(var_genes, aes(x = log(fc), y = -log10(padj), col=DE, label=DEG)) +
  geom_point() +
  geom_text_repel() +
