...
table(p.adjust(res_DESeq2$pvalue, method="BH") < 0.1,
      test_DESeq2_df_new %>% filter(gene %in% rownames(res_DESeq2)) %>% pull(padj) < 0.1) #What is this table?
```

# 3-5 Grouping of the identified DEGs

```{r}
#avg_expr <- sapply(sort(unique(meta$Run)), function(factor)
 # rowMeans(expr[,which(meta$Run == factor)]))
...
```{r, eval=FALSE}
#cut dendrogram clusters (DO NOT NEED THAT ANYMORE)
cl_DEG <- cutree(hcl_DEG, k = 9)
heatmap.2(corr_DEG, Rowv = as.dendrogram(hcl_DEG), Colv = as.dendrogram(hcl_DEG),
          trace = "none", scale = "none", labRow = NA, labCol = NA, col = rainbow,
          ColSideColors = rainbow(9)[cl_DEG])

#check avg expression level over disease state?
avg_expr <- sapply(sort(unique(meta$disease_state)), function(factor)
  rowMeans(expr[,which(meta$disease_state == factor)]))
...
#wgcna
#deconvolution
#boxplot group -> visualize per sample in each cluster
