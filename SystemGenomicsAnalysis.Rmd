...
ggplot(test_DESeq2_df_new, aes(x = log(fc), y = -log10(padj), col=DE, label=DEG)) +
  ...
  geom_text_repel(max.overlaps = 55) + # do not understand why labels on the left side dont appear, think if we want lables at all here
...
for(cluster in unique(cl_DEG[hcl_DEG$order]))
  boxplot(scaled_expr_DEG_list[[cluster]], las = 2,
          ...
          names=paste0(meta$Individual))
```

```{r}
#WGCNA / coexpression analysis for cluster 2 (highly BD) individuals

expr_DEG_BD = t(expr_DEG_list[[2]])

powers = c(c(1:10), seq(from = 12, to=40, by=2))

sft = pickSoftThreshold(expr_DEG_BD, powerVector = powers, verbose = 5)
...
#if we get really bored and lonely one could do step 3 of the tutorial and correlate external clinical traits to our modules (e.g sex)
net = blockwiseModules(expr_DEG_BD, power = 22,
...
moduleLabels_BD = net$colors
moduleColors_BD = labels2colors(net$colors)
MEs_BD = net$MEs;
geneTree_BD = net$dendrograms[[1]];

```

```{r}
#wgcna / coexpression analysis for cluster 1 (highly healthy) patients
expr_DEG_CTRL = t(expr_DEG_list[[1]])

powers = c(c(1:10), seq(from = 12, to=40, by=2))

sft = pickSoftThreshold(expr_DEG_CTRL, powerVector = powers, verbose = 5)
...
#if we get really bored and lonely one could do step 3 of the tutorial and correlate external clinical traits to our modules (e.g sex)
#deconvolution
net = blockwiseModules(expr_DEG_CTRL, power = 20,
TOMType = "unsigned", minModuleSize = 30,
reassignThreshold = 0, mergeCutHeight = 0.25,
numericLabels = TRUE, pamRespectsDendro = FALSE,
saveTOMs = TRUE,
...
moduleLabels_Control = net$colors
moduleColors_Control = labels2colors(net$colors)
MEs_Control = net$MEs;
geneTree_Control = net$dendrograms[[1]];

```

```{r}
#Create module heatmaps
modules_BD_list=split(names(moduleLabels_BD),moduleLabels_BD)
modules_Control_list=split(names(moduleLabels_Control),moduleLabels_Control)
all_modules<-append(modules_BD_list,modules_Control_list)
...
```
```{r}
#boxplots for different modules: 1,2, 3 BD upregulated; 4,5 healthy upregulated
for (module in 1:length(all_modules)){
  expr_modules<-expr[all_modules[[module]],]
  scaled_expr_modules<-t(scale(t(expr_modules)))
  
  boxplot(scaled_expr_modules, las = 2,
          main = paste0("Cluster ",module),
...
plotEnrichment(genesets_celltype_list[["GOBP_ION_TRANSPORT"]],
               scores_ordered) + labs(title="GOBP_ION_TRANSPORT")
