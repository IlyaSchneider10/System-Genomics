...
```{r}
#WGCNA / coexpression analysis for BD individuals

expr_DEG_BD = t(expr[all_DEG2$gene, 1:6])
...
net = blockwiseModules(expr_DEG_BD, power = 28,
...
```{r}
#wgcna / coexpression analysis for control patients
expr_DEG_CTRL = t(expr[all_DEG2$gene, 7:12])
...
#if we get really bored and lonely one could do step 3 of the tutorial and correlate external clinical traits to our modules (e.g sex)
#deconvolution
net = blockwiseModules(expr_DEG_CTRL, power = 26,
TOMType = "unsigned", minModuleSize = 30,
reassignThreshold = 0, mergeCutHeight = 0.25,
numericLabels = TRUE, pamRespectsDendro = FALSE,
saveTOMs = TRUE,
...
for (module in module_list[c(2:length(module_list))]){ #we skip the first module since it contains the genes that have not been assigned a module
...
write.table(meta_genes[meta_genes$expressed, "ensembl_gene_id"],
            ...
            quote = F, row.names = F, col.names = F)
#table for background for all DE genes
write.table(meta_genes[meta_genes$ensembl_gene_id_version %in% all_DEG2$gene, "ensembl_gene_id"],
            file = "genes_DE.txt",
            quote = F, row.names = F, col.names = F)
...
#plot GSEA results of one gene
plotEnrichment(genesets_celltype_list[["DESCARTES_FETAL_CEREBELLUM_MICROGLIA"]],
