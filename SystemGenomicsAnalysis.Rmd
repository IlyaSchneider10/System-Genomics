...
```{r}
avg_expr <- sapply(sort(unique(meta$disease_state)), function(disease_state)
  rowMeans(expr[,which(meta$disease_state == "Control")]))
corr_DEG <- cor(avg_expr[res_DESeq2$gene[res_DESeq2$DE],], method = "spearman")
