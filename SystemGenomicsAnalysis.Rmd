...
```{r}
#wgcna / coexpression analysis for consensus genes between both subsets - will have to elaborate 
options(stringsAsFactors = FALSE)
allowWGCNAThreads()


expr_DEG_list=list(list(as.data.frame(expr_DEG_BD)),list(as.data.frame(expr_DEG_CTRL)))
names(expr_DEG_list[[1]])<-"data"
names(expr_DEG_list[[2]])<-"data"

setLabels = c("BD", "Control")
...
for (set in 1:nSets)
powerTables[[set]] = list(data = pickSoftThreshold(expr_DEG_list[[set]]$data, powerVector=powers,
verbose = 2)[[2]]);
collectGarbage();
# Plot the results:
colors = c("black", "red")
...
expr_DEG_list, power = 26, minModuleSize = 30, deepSplit = 2,
...
```{r}
# Relating consensus modules to BD set-specific modules
```
