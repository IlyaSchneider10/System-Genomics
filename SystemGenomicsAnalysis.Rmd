...
#PCA_3D <- 
plot_ly(data.frame(pca$x, meta), x = ~pca$x[, "PC1"], y = ~pca$x[,"PC2"], z = ~pca$x[,"PC3"], color = meta$disease_state, colors=c( "brown3", "turquoise2")) %>%add_markers(size = 40)%>%
 layout(scene = list(xaxis = list(title = 'PC1'),
                    yaxis = list(title = 'PC2'),
                    zaxis = list(title = 'PC3')))

htmlwidgets::saveWidget(as_widget(plot), "PCA_3D.html")
library(animation)

# Use the saveGIF function from the animation package to create a rotating GIF of the plot
'saveGIF({
  for (i in 1:360) {
    plot %>% layout(scene = list(camera = list(up = list(x = 0, y = 0, z = 1),
                                         center = list(x = 0, y = 0, z = 0),
                                         eye = list(x = cos(pi*i/180), y = sin(pi*i/180), z = 0.5))))
    Sys.sleep(0.1)
  }
},  movie.name="PCA.gif", fps=5/720L)'
...
ggplot(res_DE, aes(x = log(fc), y = -log10(padj), col=DE, label=DEG)) +
  ...
  geom_vline(xintercept=c(log2(2), 0), col="#303030", linetype="dotted") +
...
plot(-log10(res_DESeq2$pvalue),
     ...
     xlab = "-log10(pval DESeq2)", ylab = "-log10(pval DE)", pch=16, ylim=c(0,10))
...
plot_boxplots_modules<- function(module_list){
  ...
  for (module in 1:length(module_list)){
...
#filter for name, padj, NES, size
write.csv(GO_paper[,c(1,3,6,7)], here("GO_paper.csv"), row.names=FALSE)
